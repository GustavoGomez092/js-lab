...
| 9e | A webview generation on the lifecycle wire | Each webview now carries the generation it belongs to, and `ready`/`exit` from an older one are ignored rather than acted on — so closing a tab and opening another can no longer report the replacement ready before it is, or crashed when it is healthy. Verified still reproducing at HEAD before the fix, and proven by node-identity assertions. | **merged `e72e70f`** |
| 9f | The vendored polyfill defects | Six findings, all verified: `querystring.unescape` throwing where Node returns the input; `instanceof` identity broken across `stream`/`events`/`buffer` because the generator flattens private copies of shared built-ins; the virtual `process` unbound in vendored sources; a `process` environment snapshot reaching the shared vendor cache; **`process.cwd()` disagreeing with where files actually resolve** and both disagreeing with `bun`; and a bridged command's default environment missing the runner's layered env. | **merged `f8762a8` — 5 of 6 fixed; item 2 remains OPEN** |
| 10 | Sync polyfills | The §5.13 bundled modules and the `process`/`os` snapshots. | merged |
| 11 | The async Node bridge | `fs/promises`, callback `fs`, `child_process` over an RPC bridge, with **full user permissions matching `bun`** and relative paths resolving against `workingDirectory ?? dataDir`; `*Sync` and the unsupported modules throw the spec's messages, each naming its own async counterpart. Was skipped entirely and re-instated. | **merged `a4f83e8`** |
...
2. **R-M4-T11-1** — `browser-node` actually provides the Node APIs it offers. **Reopened, then genuinely satisfied — this time on end-to-end evidence.** Closed by Task 16 (merged `8290252`), which both *found* the defect and fixed it: an e2e scenario now reads a file through `fs/promises` with a relative path resolving against the tab's working directory (`read:from-the-working-directory`) and runs a command whose stdout returns to the page (`exec:bridged`), in a real built app.
   - **What was actually broken:** `isHostToWebMessage()` in `host-bridge.ts` enumerated `stop`/`dispose`/`run`/`mute`/`expand` and all four `fetch*` variants but **none of the five `node*` ones**, so every Node reply hit `default: return false`. Main really ran the call and really sent the reply; the page discarded it. And because a rejected message correctly must not advance `lastInboundSeq`, **the connection then wedged for all later host→page traffic** — which is why `fetch` worked only if used first.
   - **A Task 9d regression against Task 11's wire, and neither task was at fault.** 9d merged first and enumerated the union as it then stood; Task 11 subsequently added the `node*` variants. **Both sides are unit-tested with fakes, so no unit test on either side could see the gap.**
...
**Added after the fact (ruling R-M4-CR-1).** Six independent, well-evidenced defects raised by CodeRabbit on PR #3 and verified against HEAD by the controller — **two of which (items 5 and 6) the reviewer marked "✅ Addressed" and which were not fixed.** Each is small; they are batched into one task and one review seat rather than four dispatches. **Sequencing: after Task 9b merges** — three of the four are in `packages/runner-web`, which is 9b's live file surface.

**Files:**
- Modify: `apps/desktop/src/main/runtimes/bun-adapter.ts` (1), `packages/runner-web/src/console-hook.ts` (2), `packages/runner-web/src/handles.ts` (3), `packages/runner-web/src/host-bridge.ts` (4), `apps/desktop/src/main/runs/run-coordinator.ts` (5), `apps/desktop/src/main/bundling/vendor-cache.ts` and `apps/desktop/src/main/runtimes/web-adapter.ts` (6)
- Test: the matching test files in `apps/desktop/test/runtimes/`, `apps/desktop/test/runs/`, `apps/desktop/test/bundling/` and `packages/runner-web/test/`

**Two of these six were reported by CodeRabbit as already fixed and are not** (items 5 and 6). Verify every one against HEAD rather than against a commit message, a resolution marker, or this plan — including the four not flagged that way.

1. **`stop()` never settles when the runner exits cleanly.** `BunRunSession.stop()` stores its resolver in `#stopSettle`, which is invoked **only** from the `"state" === "stopped"` message handler. `#onExit()` calls `clearTimeout(this.#stopTimer)` first — destroying the `stopGraceMs` fallback that is the only other thing that resolves it — then takes the clean-exit path into `#reportTerminal`, which touches `runLock` and `sink.state` and **never settles `#stopSettle`**. Resolve and clear it on every terminal exit path. *Severity is latent, not live:* the sole caller today is `void run.handle.stop()` in `run-coordinator.ts`, so nothing hangs yet — but the `RunHandle.stop(): Promise<void>` contract is violated and the first consumer that awaits it will hang. A fixture for the scenario already exists (`test/runs/fixtures/exit-on-stop-runner.ts`). **The test must actually await the contract** — that is precisely why a fully green suite never saw this. (No figure is given here on purpose: a test total quoted in a plan goes stale within days, as several in this document did.)
...
5. **A run can be declared unresponsive the instant it starts.** `lastHeartbeatAt` is stamped at run creation and refreshed only on `heartbeat` (and by the dialog's "Wait"). Bundling happens in between. `#checkHeartbeats` guards with `if (!run.handle || …) continue`, which **defers** the judgement until the handle attaches but does not refresh the stale stamp — so the moment `attached:` fires, a creation-time stamp older than `unresponsiveTimeoutMs` is evaluated and the user sees the unresponsive prompt for a run that just began. Stamp `lastHeartbeatAt` in the `attached:` callback. **Note: CodeRabbit marked this finding "addressed" and it is not** — verify against HEAD, not against the marker.
6. **The vendor cache key omits the runtime.** `vendorKeyFor` returns `vendorCacheKey(hashBunLock(lockText), imports)` — the key is the lock hash plus the sorted import set, and nothing else. But `bundleVendorForWeb` receives the runtime and feeds it to `nodePolyfills`, where `browser` rejects Node builtins that `browser-node` resolves through polyfills. So a `browser-node` chunk can be served to a `browser` tab on a cache hit, and the chunk may also embed a per-working-directory `process` environment snapshot that neither the key nor `vendorChunkFitsTab` accounts for (that guard covers working-directory *shadowing* only, and returns `true` outright when there is no working directory). Include the runtime in the key and add a cross-runtime key test. **Also marked "addressed" and also not** — what those commits actually landed was the read-side working-directory guard, a different finding whose marker was correct.
...
- [ ] Steps: one failing test per defect **that fails for the stated reason**, then each fix, then the gates. **Counts: measure your own baseline and report `baseline N → after M`.** Six fixes, roughly six to ten tests; no absolute total is given here on purpose.
...
**Re-verified at the tip after Task 9c merged, because 9c changed one of these files.** Both premises still hold: `webRunner.ready` and `webRunner.exit` still take `WebRunnerTabParams` (`packages/rpc-schema/src/ui-rpc.ts:467,469`), so the wire genuinely carries only the tab id; and `createUiWebviewSource` still keeps one `Entry` per tab in a `Map` (`webview-source.ts:48`). **What did change is `apps/ui/src/output/webview-host.ts`:** Task 9c added a `wanted` set — every tabId Main has ever asked for, populated by `webRunner.ensure` and `webRunner.reload` — and **ready-forwarding is now gated on it**: `if (wanted.has(tabId)) api.webRunnerReady(tabId)`. Your generation has to travel through that same path, so read that file before designing the token: a generation that does not reach the `wanted` gate will be dropped exactly where 9c's fix suppresses the `ready-NO-HOST` load.
...
### Task 9f: The vendored polyfill defects

**Added after the fact, and this task exists because of a near-miss.** Items 1–4 were raised by CodeRabbit on PR #3 and **verified live against HEAD by the controller**; items 5 and 6 were found and **handed back deliberately by Task 11's implementer**, which surfaced them while relaxing the filesystem scoping and correctly declined to fix code its brief told it not to disturb. The ledger recorded that they would be folded into Task 9d; when 9d's section was actually written, only two of the findings it names made it in, and these four were left in the ledger owned by nobody. That is the same mechanism that lost Task 11 for days. They are written down here so they cannot be lost again.

**Files:**
- Modify: `packages/runner-web/scripts-src/generate-vendor.sh` and the regenerated files under `packages/runner-web/src/polyfills/vendor/`, `apps/desktop/src/main/bundling/polyfill-plugin.ts`, `apps/desktop/src/main/bundling/bundler.ts`
- Test: `apps/desktop/test/bundling/polyfill-plugin.test.ts`, `apps/desktop/test/bundling/bundler.test.ts`

**Verify each against HEAD before fixing it.** Each entry below was confirmed by the controller, but a brief is a claim like any other. If one no longer reproduces, say so and do not invent a change to justify the entry.

1. **`querystring.unescape` throws where Node returns the input.** The generated module exports `globalThis.decodeURIComponent` as `unescape`, so malformed input such as `"%"` throws `URIError`; Node's `querystring.unescape("%")` returns `"%"`. Confirmed: the file's own header says the entry adds escape/unescape via the global functions, and **there is no `catch` token anywhere in the generated file**. Implement the tolerant fallback in the **generator**, regenerate, and test the exported `unescape` with malformed percent input. A Node-compatibility divergence in the runtime whose entire purpose is Node compatibility.
2. **Shared built-in constructor identity is broken across `stream`, `events` and `buffer`.** `generate-vendor.sh`'s `build()` is `bun build "$entry" --target=browser --format=esm --minify --outfile=…` with **no `--external` at all**, so every vendor file flattens its own private copy of shared built-ins; `stream-browserify.js` embeds `EventEmitter` while `events.js` is built separately with its own. `new Readable() instanceof EventEmitter` therefore compares two distinct constructors. Fix in the generation script so shared built-ins stay external and resolve through the browser-node module table; regenerate; test constructor identity across all three. This passes every unit test today and breaks real library code doing `instanceof`.
3. **The virtual `process` is unbound in vendored sources.** `polyfill-plugin.ts`'s `onLoad` returns vendored text **raw** (`return { contents: vendor, loader: "js" }`) with nothing prepended, while `process` *is* resolvable in that same namespace. `assert.js` references `process.env` and `path-browserify.js` references `process.cwd`. Confirmed **structurally** — the precondition is proven; the throw was not executed. Prepend the import to the vendor source at load time rather than installing a page global (this codebase deliberately avoids new page globals).
4. **A `process` environment snapshot can reach the shared vendor cache.** `nodePolyfills` resolves `process` as a virtual module, so it never enters the resolved closure or `workingDirectoryImports`, leaving the vendor build cacheable — while neither the cache key nor `vendorChunkFitsTab` accounts for the tab's working directory or its generated environment snapshot. A vendor package that imports `process` can therefore persist one tab's snapshot and serve it to another. **Overlaps Task 9d's item 6** (the runtime missing from the cache key): 9d was dispatched before this task existed, so **reconcile against whatever 9d actually landed** rather than assuming, and say in your report which half was already closed. 9d fixed the key and **explicitly did not** touch this; `vendorChunkFitsTab` still returns `true` outright when there is no working directory.
5. **`process.cwd()` disagrees with where files actually resolve, and both disagree with `bun`.** `polyfill-plugin.ts`'s `processModuleSource` sets `cwd: ctx.workingDirectory ?? process.cwd()` — **Main's own process cwd**, not the app's data directory. Task 11 made `fs` and `child_process` resolve relative paths against `workingDirectory ?? dataDir`, matching the Bun runner (`runner-config.ts:69`). So for a tab with **no working directory**, `process.cwd()` now reports one place, `fs.readFile("notes.txt")` reads from another, and `bun` would use a third. Task 11 surfaced this rather than causing it, and correctly declined to fix it: the repair means plumbing `dataDir` through `BundleOptions`, `VendorBundleOptions`, `BrowserNodeContext` and both `bundler.ts` call sites. **No test covers the no-working-directory case at all** — that is why it went unnoticed, and the fix must add one that pins all three to the same base.
...
**A third carried finding is NOT yours — it was Task 9e**, now **merged (`e72e70f`)**: `webRunner.ready`/`.exit` gained a generation, so a late event from a destroyed webview no longer reaches its replacement. Nothing to record as open.

**Four more findings arrived after this section was written. Each needs recording, and none needs fixing by you.** They exist only because tasks ran the built app; if you omit them, nothing else will catch them.

3. **An open defect: shared built-in identity is broken across `stream`, `events` and `buffer`.** Task 9f fixed five of its six items and **deliberately left this one**, having measured that the prescribed fix does nothing: `--external` is silently ignored for Node builtin *names* under `--target=browser` (byte-identical output), and the plugin alternative drops CommonJS `require()` dependencies and produces something broken at runtime. So `new Readable() instanceof EventEmitter` can be `false`, and a `Buffer` from one module may not be recognised by another. A real fix means building the shared built-ins as **one module graph** — a vendor-layer redesign. **Record it as a known limitation with that reasoning**; the measurements are in `generate-vendor.sh`'s own header.
4. **`OfflineAudioContext` drives no audio indicator.** It is referenced **nowhere** in `packages/runner-web` (0 hits, against 8 for `AudioContext`), so an offline render is untracked. Only a live `AudioContext` or a playing media element lights the indicator — and a live context starts `suspended` under the autoplay policy until a gesture, which the runner already accounts for by registering on `state === "running"`. **EX-35 must not claim more than that.**
5. **The `scheduler` pin in the opt-in e2e suite is decorative.** `three` is genuinely version-checked (`if (version !== THREE_VERSION) throw`) and react/react-dom are named in their republish calls, but scheduler is republished via `copyInstalledPackage("scheduler", …)`, which takes whatever version this repo has installed. Say "inherits the installed version", not "pinned".
6. **`bun run e2e` does NOT build — it tests whatever bundle is already on disk, however old.** `packages/e2e/package.json`'s `e2e` script is `bun test ./scenarios` and nothing more; `src/app.ts` builds only when it has to resolve a path itself. **A stale bundle produces fast, confident, entirely misleading failures**: running the suite against a build that predated this milestone's work gave **60 pass / 9 fail**, with every browser-runtime scenario refused at `Command is disabled: runtime.browser` in ~1 s, because `AVAILABLE_RUNTIMES` inside that bundle was still `["bun"]`. It looks exactly like a regression in core functionality and is not one. **The QA checklist must say: build first (`hutch run build:dev`), then run the suite** — and must warn that the failure text, not the timings, is what identifies this.
7. **The opt-in suite needs a local `three@0.184.0` and fails loudly without one.** It republishes from Bun's on-disk cache, overridable with `JSLAB_E2E_THREE_DIR`, and **never** touches the public registry. That was deliberate — a silently-skipped exit criterion is worse than a loud failure — but the QA checklist must tell a person how to satisfy it rather than leaving them with a red suite and no explanation.

**Two gaps that WERE yours and are now Task 9g's** (so do not duplicate them, but do check what actually shipped before writing the rows): `console.log(element)` rendering as an empty entry, and the Web View toggle having no command id. **Parity WV-01 and TF-19 depend on the second**, so verify whether 9g landed before marking either row automated.
