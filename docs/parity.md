...
| EX-22 | Runtime environments per tab: Browser & Node.js (default), Node.js, Browser | Docs, CL 4.0.0 | `bun` (**default**, D13), `browser-node`, `browser` | §5.2 | M1/M4 | E | 📝 Node.js → Bun (D2); **JSLab defaults to `bun`, not `browser-node` (D13)**; `browser-node` Node APIs are async-only for fs/child_process (§5.13). All three run end to end: `packages/e2e/scenarios/web-runtime.test.ts` |
