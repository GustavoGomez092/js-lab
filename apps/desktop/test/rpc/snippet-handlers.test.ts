...
function setup(options: { file?: string; chosen?: string[]; savePath?: string | null } = {}) {
  ...
  /**
   * The real reader (src/main/fs/bounded-read.ts) refuses from the opened handle's `fstat`, before it
   ...
   */
  const readBoundedFile = mock(async (path: string, maxBytes: number) => {
  ...
  return { handlers, imported, exported, written, readBoundedFile, current: () => stored };
}
...
describe("snippet handlers (spec §13.1, §13.4, §18)", () => {
  ...
  test("a file too large to be a library is refused, at the documented cap", async () => {
    ...
    // in test/fs/bounded-read.test.ts, which this fake cannot show.
    const padded = `${library([record({ id: "i1", name: "brandnew" })])}${" ".repeat(5 * 1024 * 1024)}`;
    const { handlers, imported, current, readBoundedFile } = setup({ file: padded });
    handlers.messages["snippets.importDialog"]({});
    await flush();
    expect(imported).toEqual([{ ok: false, reason: "tooLarge", detail: strings.snippets.tooLarge }]);
    expect(readBoundedFile.mock.calls[0]).toEqual(["/tmp/library.json", MAX_SNIPPETS_FILE_BYTES]);
    expect(current()).toEqual([record()]);
  });
