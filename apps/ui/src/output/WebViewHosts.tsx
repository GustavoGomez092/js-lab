 ...
 *
 * Every tab whose runtime isn't `bun` **and whose Web View toggle has been switched on at least once** gets a
 * `WebViewTile`, keyed by `tabId` so React never remounts it across a tab switch (N4, carried from Task 8: a host
 * keyed purely on runtime meant twenty `browser` tabs meant twenty live webviews, including tabs whose toggle was
 * never touched -- created lazily instead, on first enable). Each one **always** portals into this component's own
 * permanent `parkingNode` (see `WebViewTile.tsx`'s doc comment for why: portaling into `OutputTiles`'s own
 * placeholder instead was fix round 1's first attempt, and it failed -- that placeholder is a DOM child of a
 * component that is allowed to unmount, and removing a DOM node destroys everything physically nested inside it,
 * portaled content included). The tab named by `dock.tabId` gets `dockNode={dock.node}` and `docked` so it
 * visually tracks that placeholder's box; every other tab gets `dockNode={null}`, `docked={false}` and collapses
...
export function WebViewHosts({ store, dock }: { store: AppStore; dock: WebviewDock | null }) {
  ...
  // N4: which tabIds have ever had their Web View toggle on. Once added here a tabId is never removed while its
  // tab stays open -- even if the toggle goes back off -- which is what makes host creation a one-way, "first
  // enable" event rather than something a later disable could undo (the Task 8 never-unmount invariant this must
  // not weaken). A tabId is only ever dropped when the tab itself closes (or its runtime becomes `bun`, which
  // removes Web View capability entirely -- the same case Task 8's own filter already excluded).
  ...
  useEffect(() => {
    ...
    setEverEnabled((previous) => {
      ...
      for (const id of previous) {
        ...
        else changed = true; // the tab closed, or lost web capability: drop its host from the render list
      }
