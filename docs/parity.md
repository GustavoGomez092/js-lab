...
| ID | RunJS capability | Source | JSLab | Spec | MS | Verify | Status |
|---|---|---|---|---|---|---|---|
| ED-01 | Line Wrap | Docs | `editor.lineWrap` | §6.1 | M2 | E | ⬜ |
| ED-02 | Vim Keys | Docs | `monaco-vim`; Run works in normal mode (#652) | §6.3 | M2 | M | ⬜ |
| ED-03 | Close Brackets | Docs | `editor.closeBrackets` | §8 | M2 | M | ⬜ |
| ED-04 | Font Ligatures | Docs | `appearance.fontLigatures` | §8 | M2 | M | ⬜ |
| ED-05 | Line Numbers | Docs | `editor.lineNumbers` | §8 | M2 | M | ⬜ |
| ED-06 | Invisibles | Docs | `editor.invisibles` | §8 | M2 | M | ⬜ |
| ED-07 | Active Line | Docs | `editor.activeLine` | §8 | M2 | M | ⬜ |
...
| ED-15 | Find / Replace | CL 1.11.0, Docs | Monaco find widget | §6.5 | M2 | M | ⬜ |
| ED-16 | Go to line | Strings | `Ctrl+G` | §6.5 | M2 | M | ⬜ |
| ED-17 | Toggle line/block comment | CL 2.3.0 | Same | §6.5 | M2 | M | ⬜ |
| ED-18 | Tab indents selection | CL 1.11.0 | Monaco default | — | M2 | M | ⬜ |
| ED-19 | Matching bracket / word highlights; fold markers | CL 1.10.0 | Monaco defaults | — | M2 | M | ⬜ |
| ED-20 | Editor context menu (incl. Create Snippet…) | CL 1.11.0, Strings | Same | §13.1 | M2/M5 | M | ⬜ |
| ED-21 | Line-editing shortcuts (Docs shortcut table) | Docs | Default keybindings table | §6.5 | M2 | E | ⬜ |
| ED-22 | Format code with Prettier (Alt+Shift+F) | Docs | Prettier worker, minimal edits | §6.4 | M2 | U, E | ⬜ |
| ED-23 | Auto Format on run | Docs | `run.formatOnRun` | §6.4 | M2 | E | ⬜ |
| ED-24 | Formatting options (print width, tab width, semicolons, quotes, quote props, JSX quotes, trailing commas, bracket spacing, arrow parens) | Docs | All `prettier.*` + Use Tabs (#728) + bracketSameLine | §8 | M2 | U | ⬜ |
| ED-25 | Large paste guard | CL 2.6.0 | 5 MB confirmation | §6.3 | M2 | M | ⬜ |
| ED-26 | Install assist for missing imports | CL 1.10.0, 2.7.4 | Code action + runtime error fix | §11.4 | M3 | E | ⬜ |
| ED-27 | Zoom (Cmd =/−/0) scales editor, output and sidebar | Docs, CL 3.2.0 | `appearance.uiScale` | §6.5 | M2 | M | ⬜ |
...
| ID | RunJS capability | Source | JSLab | Spec | MS | Verify | Status |
|---|---|---|---|---|---|---|---|
| TF-01 | Multiple tabs; new/close shortcuts | Docs, CL 2.0.0 | Same | §7.3 | M2 | E | ⬜ |
| TF-02 | Tab title from first line; Edit Tab Title | Docs, CL 2.12.0 | Same | §7.3 | M2 | E | ⬜ |
| TF-03 | Middle-click closes tab | CL 2.3.0 | Same | §7.3 | M2 | M | ⬜ |
| TF-04 | Close Other Tabs / Close Tabs to the Right | CL 2.10.1 | Same | §7.3 | M2 | E | ⬜ |
| TF-05 | Reopen closed tab | CL 2.11.0 | Stack of 20 | §7.3 | M2 | E | ⬜ |
| TF-06 | Jump to tab (Cmd+1–9), next/previous tab | Docs | Same | §6.5 | M2 | E | ⬜ |
| TF-07 | Hide tab bar when single tab | CL 2.1.0, Docs | `view.tabBarForSingleTab` | §8 | M2 | M | ⬜ |
| TF-08 | Confirm Close | CL 2.4.0, Docs | `tabs.confirmClose` | §7.3 | M2 | E | ⬜ |
| TF-09 | Prompt to save a modified file on close | CL 2.7.1 | Same | §7.3 | M2 | E | ⬜ |
...
| TF-18 | Activity bar: Run, Stop, Snippets, NPM, AI Chat, Settings | Docs | Same | §7.1 | M2 | E | ⬜ |
| TF-19 | Status bar: runtime, language, web view toggle, split toggle | Docs | Same + WD chip, run state, vim mode | §7.1 | M2 | E | ⬜ |
| TF-20 | File associations js/jsx/ts/tsx | #620 | + mjs/cjs/mts/cts via Info.plist patch | §4.6 | M6 | M | ⬜ |
| TF-21 | Closing last tab quits the app | #650 | Opens a fresh tab instead | §7.3 | M2 | E | 📝 intentional improvement |

## 7. Settings, themes, i18n

| ID | RunJS capability | Source | JSLab | Spec | MS | Verify | Status |
|---|---|---|---|---|---|---|---|
| ST-01 | Settings window with tabs (General, Editor, Formatting, Appearance, AI, NPM, Advanced) | Docs, Strings | + Keybindings, Build | §8 | M2 | E | ⬜ |
| ST-02 | Tooltips/help text on options | CL 2.5.0 | Help text per setting | §8, §17 | M2 | M | ⬜ |
| ST-03 | Theme picker + Themes menu with icons | Docs, CL 2.2.2 | Themes menu + Appearance tab | §9 | M2 | E | ⬜ |
| ST-04 | Themes (Dracula default, others) | Docs | 19 built-in themes, all free | §9.2 | M2 | M | ⬜ |
| ST-05 | Font picker: pre-loaded coding fonts + installed system fonts | Docs | Bundled fonts + `systemFonts` adapter | §9.4 | M2 | M | ⬜ |
| ST-06 | Font size | Docs | `appearance.fontSize` | §8 | M2 | M | ⬜ |
| ST-07 | Tab Bar / Activity Bar / Status Bar toggles | Docs | Same | §8 | M2 | M | ⬜ |
...
| ST-10 | Copy Debug Log to Clipboard | Strings | Help → Copy Debug Log (redacted) | §20 | M2 | I | ⬜ |
...
| XT-02 | Custom keybindings UI + `keybindings.json` | #446, #636 | §6.5 | M2/M5 | U, E | ⬜ |
