...
/** Unbounded probe, used only for the finite, non attacker-scalable candidate lists (manifest entries, requested specifiers). */
...
/** Budgeted, deduped probe used inside `closure()`'s walk, where candidates are driven by scanned file content. */
...
async function closure(
  ...
  const probes = { left: input.maxFiles * PROBE_BUDGET_PER_FILE };
  ...
  probeLoop: while (queue.length > 0) {
    ...
    for (const match of content.matchAll(REFERENCE_PATH)) {
      // I-1: drop lexically out-of-root candidates before ever probing them.
...
export async function collectLocalTypes(
  ...
  if (wd === "" || wd === ".") return { files: [], packages: [], truncated: false };
  const roots: string[] = [];
