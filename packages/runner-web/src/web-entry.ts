 ...
 * `apps/desktop/hutch.config.ts` bundles it (`--format iife`) to `dist/runner/web-bootstrap.js`, which
 * `electrobun.config.ts`'s existing `dist/runner` → `runner` rule ships into the app.
 *
