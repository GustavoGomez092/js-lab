...
| 9f | The vendored polyfill defects | Four verified findings that were left unowned when 9d was written: `querystring.unescape` throwing where Node returns the input; `instanceof` identity broken across `stream`/`events`/`buffer` because the generator flattens private copies of shared built-ins; the virtual `process` unbound in vendored sources; and a `process` environment snapshot reaching the shared vendor cache. | queued |
...
**Added after the fact, and this task exists because of a near-miss.** All four were raised by CodeRabbit on PR #3 and **verified live against HEAD by the controller**. The ledger recorded that they would be folded into Task 9d; when 9d's section was actually written, only two of the findings it names made it in, and these four were left in the ledger owned by nobody. That is the same mechanism that lost Task 11 for days. They are written down here so they cannot be lost again.
