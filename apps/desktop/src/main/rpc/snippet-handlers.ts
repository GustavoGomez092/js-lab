...
import { FileTooLargeError } from "../fs/bounded-read";
  ...
  return {
    ...
    messages: {
      "snippets.importDialog": message(emptyParamsSchema, "snippets.importDialog", async () => {
        ...
        let text: string;
        try {
          // R-M5b-S2: the cap belongs to the read. `readBoundedFile` refuses from the opened handle's `fstat`, before it
          ...
          text = await deps.readBoundedFile(path, MAX_SNIPPETS_FILE_BYTES);
        } catch (error) {
